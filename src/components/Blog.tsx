...
import { Search, User, Clock, BookOpen, ChevronRight, X } from "lucide-react";

const SITE_URL = "https://www.diavoxtech.in";

export default function Blog() {
  const { theme, blogs } = useStore();
  ...
  React.useEffect(() => {
    ...
    if (!readableBlog) return;

    const blogUrl = `${SITE_URL}/blog/${readableBlog.slug}`;
    const description = (readableBlog.content || "")
      .replace(/\s+/g, " ")
      .slice(0, 155);
    ...
    const blogSchema = {
      ...
      "@type": "BlogPosting",
      headline: readableBlog.title,
      description,
      image: readableBlog.image_url || `${SITE_URL}/og-image.png`,
      author: {
        "@type": "Organization",
        name: readableBlog.author_name || "Diavox Tech",
      },
      publisher: {
        "@type": "Organization",
        name: "Diavox Tech",
      ...
      url: blogUrl,
      datePublished: readableBlog.created_at || new Date().toISOString(),
      dateModified: readableBlog.created_at || new Date().toISOString(),
      articleSection: readableBlog.category,
    };
    ...
    return () => {
      const current = document.getElementById("blogposting-jsonld");
  ...
  return (
    <section
      id="blog"
      className={`py-20 md:py-28 text-left ${
        theme === "dark" ? "bg-slate-950 text-white" : "bg-white text-slate-900"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div
          className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end justify-between mb-12"
          id="blog-header-block"
        >
          <div className="md:col-span-7 space-y-4">
            <p className="text-xs font-mono uppercase tracking-widest text-cyan-500 font-bold">
              DIAVOX KNOWLEDGE PORTAL
            </p>
            ...
            <h2 className="text-3xl sm:text-4xl md:text-5.5xl font-display font-light tracking-tight leading-tight">
              ...
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-500 via-amber-250 to-purple-400 font-normal italic">
                SEO Insights
              </span>
            </h2>

            <p className="text-sm opacity-75 font-light">
              Practical guides on websites, UI/UX, SEO, AI automation, and business growth from the Diavox Tech team.
            </p>
          </div>
        ...
        {filteredBlogs.length === 0 && (
          <div className="text-center py-20 p-6 border border-dashed rounded-2xl dark:border-slate-800 border-slate-200">
            <p className="text-sm font-mono opacity-60">
              No articles matched "{search}". Check spelling or try other categories.
            </p>
          </div>
        )}
        ...
        {readableBlog && (
          ...
          >
            <div
              className={`w-full max-w-lg md:max-w-2xl rounded-2xl overflow-hidden border shadow-2xl relative max-h-[90vh] flex flex-col ${
                theme === "dark"
            ...
            >
              ...
              <div className="overflow-y-auto flex-1">
                <div className="aspect-video relative bg-slate-950">
                  <img
                    src={readableBlog.image_url}
                    alt={readableBlog.title}
                ...
                <div className="p-6 sm:p-8 space-y-6">
                  <div className="flex flex-wrap items-center justify-between gap-3 text-xs font-mono opacity-50 border-b dark:border-slate-800 border-slate-100 pb-4">
                    ...
                    <span className="inline-flex items-center space-x-1.5">
                      <Clock size={13} />
                      <span>{readableBlog.read_time}</span>
                    </span>
                  </div>

                  <h1 className="text-xl sm:text-2xl font-display font-extrabold leading-tight">
                    {readableBlog.title}
                  </h1>
                  ...
                  <div className="text-sm opacity-85 leading-relaxed font-light space-y-4">
                    ...
                    <p className="font-medium text-cyan-500">
                      Diavox Tech — Crafting Digital Experiences Worldwide.
                    </p>
                  </div>
                </div>
              </div>
